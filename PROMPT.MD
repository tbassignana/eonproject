...
4. **Complete the Next Item**:
   ...
   - Ensure the implementation aligns with the game's requirements: 3rd person perspective, online single-player (solo instance) mode, multiplayer instanced sessions via SpaceTimeDB (e.g., real-time player syncing, instance creation/joining).
5. **Write Tests**:
   ...
   - Tests should cover functionality, edge cases, performance (especially for multiplayer), and SpaceTimeDB interactions.
...
- A final integration test suite passes, confirming single-player and multiplayer functionality.

## Game Specifications
- **Core Features**:
  - 3rd person camera and controls (movement, jumping, interacting with environment).
...
- **Technical Stack**:
  - Frontend: Unreal Engine 5.5.4 (C++ only, NO Blueprints) for 3D rendering and player input.
  ...
  - Deployment: Build for PC/Web, deploy instances to a cloud provider if needed (e.g., via Docker for SpaceTimeDB).
- **Best Practices**:
  ...
  - Performance: Optimize for real-time multiplayer (e.g., efficient data syncing).
  - Security: Basic measures like input validation; avoid exposing sensitive data.
  - Documentation: Inline comments, README.md with setup instructions.
...
2. Install and configure development tools (Unreal Engine 5.5.4, SpaceTimeDB SDK with user tbassignana).
3. Implement basic 3rd person player controller in C++.
4. Add core gameplay features (e.g., environment, combat).
...
9. Deploy a demo instance.
10. Final polish and full test run.
